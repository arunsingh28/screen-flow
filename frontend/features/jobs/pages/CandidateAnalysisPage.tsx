...
const CandidateAnalysisPage = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            setLoading(true);
            try {
                // Fetch Job Details for JD Text
                try {
                    const jobData = await jobsApi.getJob(jobId);
                    setJobDescription(jobData.job_description_text || jobData.description || "No Job Description available.");
                } catch (e) {
                    console.error("Failed to fetch job details", e);
                }

                // Initial list fetch (lighter)
                ...
                if (found) {
                    // Fetch FULL details for single candidate
                    try {
                        const fullCandidateData = await jobsApi.getCV(candidateId);

                        // Merge full data
                        const mergedCandidate = {
                            ...found,
                            jdMatchData: fullCandidateData.jd_match_data,
                            githubData: fullCandidateData.github_data,
                            parsedData: fullCandidateData.parsed_data,
                            // Add any other specific fields if needed
                            parsedText: fullCandidateData.parsed_text
                        };

                        setCandidate(mergedCandidate);
                    } catch (e) {
                        console.error("Failed to fetch full CV details", e);
                        // Fallback to list data if fail
                        setCandidate(found);
                    }
    ...
    // Ensure we access score_calculation correctly from the API response
                    ...
                    <Tabs defaultValue="analysis" className="flex-1 flex flex-col overflow-hidden">
                        <div className="px-6 pt-2 border-b">
                            <TabsList className="w-full justify-start rounded-none border-b-0 p-0 h-auto bg-transparent gap-6">
                                <TabsTrigger
                                    value="analysis"
                                    className="rounded-none border-b-2 border-transparent px-4 py-3 font-medium text-muted-foreground data-[state=active]:border-primary data-[state=active]:text-primary data-[state=active]:bg-transparent data-[state=active]:shadow-none transition-none"
                                >
                                    Analysis & Report
                                </TabsTrigger>
                                <TabsTrigger
                        ...
                        <TabsContent value="analysis" className="flex-1 overflow-hidden m-0 p-0 outline-none data-[state=inactive]:hidden">
                        ...
                        <TabsContent value="debug" className="flex-1 overflow-hidden m-0 p-0 outline-none data-[state=inactive]:hidden">
                            <ScrollArea className="h-full">
                                <div className="p-6">
                                    <div className="space-y-6">
                                        <Card>
                                            <CardHeader>
                                                <CardTitle className="text-lg">Parsed Text</CardTitle>
                                                <CardDescription>Raw text extracted from the CV</CardDescription>
                                            </CardHeader>
                                            <CardContent>
                                                <div className="bg-muted/30 p-4 rounded-md text-xs font-mono whitespace-pre-wrap max-h-[300px] overflow-auto border">
                                                    {(candidate as any).parsedText || "No parsed text available."}
                                                </div>
                                            </CardContent>
                                        </Card>
                                        ...
                                        <Card>
                                            <CardHeader>
                                                <CardTitle className="text-lg">Captured JSON Data</CardTitle>
                                                <CardDescription>Full structured data used for analysis</CardDescription>
                                            </CardHeader>
                                            <CardContent>
                                                <div className="bg-muted/30 p-4 rounded-md text-xs font-mono whitespace-pre-wrap max-h-[400px] overflow-auto border">
                                                    {JSON.stringify({
                                                        jdMatchData: candidate.jdMatchData,
